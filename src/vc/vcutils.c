...
uint32_t str_hash(unsigned char *str_to_hash, size_t len_str){
    /* This function converts an array of char elements to an uint32_t integer hash.
     * Arguments:
     ...
     * Outcome:
     * An uint32_t integer will be returned as the result. Only the lower 32 bits are significant
    ...
    for (size_t i = 0; i < len_str; i++){
        ...
        hash_result += (uint32_t)(str_to_hash[i]);
