...
def conn_ms_ws():
    ...
    while True:
        try:
            ws = websocket.create_connection(config_ws["ws_host"], origin=config_ws["ms_host"])
            ...
            if config_ws["max_retry"] > -1:  # -1 is unlimited.
                if failure_count > config_ws["max_retry"]:
                    msg.output("Cannot connect to Metasmoke WebSocket.", msg.ERROR, tags=["WebSocket"])
    ...
    while True:
        try:
            ...
            try:
                ...
                msg.output("Metasmoke WebSocket response is invalid.", msg.WARNING, tags=["WebSocket"])
                continue

            if "type" in data:
                if data["type"] == "welcome":
                    msg.output("Metasmoke WebSocket welcome message received.", msg.DEBUG, tags=["WebSocket"])
                    continue
                if data["type"] == "reject_subscription":
                    msg.output("Metasmoke WebSocket subscription rejected.", msg.CRITICAL, tags=["WebSocket"])
                    raise RuntimeError("Metasmoke WebSocket subscription rejected.")
                if data["type"] == "ping":
                    msg.output("Metasmoke WebSocket ping received.", msg.VERBOSE, tags=["WebSocket"])
                    continue

            if "message" not in data:
                msg.output("Metasmoke WebSocket response does not contain message field.", msg.DEBUG, tags=["WebSocket"])
                continue

            message = data["message"]
            msg.output("Metasmoke WebSocket message received.", msg.VERBOSE, tags=["WebSocket"])

            if message["event_class"] == "Post":
                # New post created. Analyze it.
                post_id = message["object"]["id"]
                msg.output("New post {} received from Metasmoke WebSocket.".format(post_id), msg.INFO, tags=["WebSocket", "Post"])
                
                user = dataproc.get_user(message["object"]["user_link"], message["object"]["username"])
                msg.output("Author of post {} extracted as {}.".format(post_id, user), msg.VERBOSE, tags=["Post"])
            ...
            if message["event_class"] == "Feedback":
                ...
                msg.output("New feedback event for post {} received from Metasmoke WebSocket.".format(post_id), msg.DEBUG, tags=["WebSocket", "Feedback"])
            ...
            msg.output("User enforced program termination.", msg.DEBUG, tags=["Framework"])
            ws.close()
            return None
        except Exception as e:
            ...
            try:
                ...
                pass
            ws = conn_ms_ws()
            msg.output("Reconnected to Metasmoke WebSocket.", msg.DEBUG, tags=["WebSocket"])
